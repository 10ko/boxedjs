...
const filterFn = (item) => path.extname(item.path) === '.html';
...
module.exports = {
  compile: (boxedrc) => {
    ...
    fs.readdirSync(folders.pages)
      .filter((item) => {
        return item.indexOf('.yml') > -1 || item.indexOf('.yaml') > -1;
      }).forEach((item) => {
        const itemKey = item.indexOf('.yml') > -1
    ...
    const templatesRaw = klawSync(folders.templates, {filter: filterFn});
    templatesRaw.map((item) => {
      const itemKey =
          item['path'].substring(0, item['path'].length - 5)
